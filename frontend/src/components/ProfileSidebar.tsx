...
type UserStatus = "ON_DUTY" | "AWAY" | "OFF";
...
export default function ProfileSidebar() {
  ...
  const statusInfo = {
    ...
    OFF: { text: "오프라인", color: "bg-red-500" },
  };
  ...
  const fetchMyStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          status &&
          (status === "ON_DUTY" || status === "AWAY" || status === "OFF")
        ) {
          setStatus(status);
        }
      } else {
